...
class belle(unittest.TestCase):
    ...
    @classmethod#多个用例只有第一次会初始化
    def setUpClass(cls):
         login.login.login_go(cls)  # 调用login.py文件的登录封装方法
    ...
    def tearDownClass(cls):
        ...
        print("测试完成")
    def setUp(self):
        login.login.refresh(self)
        print("刷新页面，回到首页")

    def testStore(self):
        print("SC-店铺组设置-基本功能验证")
        time.sleep(1)
        ...
        # time.sleep(5)
    def testBankSet(self):
        print("SC-店铺组存现账号设置-基本功能验证")
        time.sleep(1)
        self.dir.find_element_by_xpath("/html/body/div[2]/div[1]/div[4]/span/input[1]").send_keys("SC-店铺组存现账号设置")  # 输入店铺组设置
        time.sleep(3)
        self.dir.find_element_by_xpath("/html/body/div[2]/div[1]/div[4]/span/input[1]").send_keys(Keys.ENTER)  # 回车
        time.sleep(2)
