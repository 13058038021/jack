...
class Run_testAll(unittest.TestCase):

    def test_Run_report(self):
        #指定测试用例为当前文件夹下的test_Case目录,并匹配测试脚本
        self.discover = unittest.defaultTestLoader.discover(start_dir="D:\\python\\testcase",pattern="test*.py",top_level_dir=None)
        #定义报告存储路径
        self.reportPath="D:\\python\\report\\"+"TestReport.html"
        #定义写入文件路径和方法
        fp=open(self.reportPath,"w")
        #将报告写入到定义的文件中
        self.runner=HTMLTestRunner.HTMLTestRunner(stream=fp,title=u"testreport",verbosity=2,description="testcase")
        # 执行测试用例
